...
NSString * Cothran_Michael_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Cothran_Michael_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_cothran_michael_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Cothran_Michael_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Cothran_Michael_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_cothran_michael_project2 = [[Cothran_Michael_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_cothran_michael_project2];
	
	NSString *_cothran_michael_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _cothran_michael_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_cothran_michael_project2 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Cothran_Michael_Project2$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.cothran_michael_project2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.cothran_michael_project2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
