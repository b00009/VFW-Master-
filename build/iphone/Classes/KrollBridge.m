...
NSString * Cothran_Michael_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Cothran_Michael_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_cothran_michael_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Cothran_Michael_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Cothran_Michael_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_cothran_michael_project1 = [[Cothran_Michael_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_cothran_michael_project1];
	
	NSString *_cothran_michael_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _cothran_michael_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_cothran_michael_project1 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Cothran_Michael_Project1$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.cothran_michael_project1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.cothran_michael_project1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
